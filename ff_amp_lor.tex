Before diving into the details regarding one how to extract desired observables form lattice QCD, it is worth defining in details the infinite-volume quantities we will studying. Our aim is two-fold:
	\begin{itemize}
	...
	\item Constraining this in an large kinematic window, we can then analytically continue onto the $\rho$ poles of the initial and final states, and obtain the three form-factors of the $\rho$. 
	\end{itemize}
...
\begin{center}
\subfigure[]{
\includegraphics[width=.8\textwidth]{flowchart2}}
...
\end{align}
where we use the four-dimensional units vectors
\begin{align}
\hat{e}_\pm&=\mp\frac{1}{\sqrt{2}}(0,1,\pm i,0),
\\
\hat{e}_0&=(0,0,0,1).
\end{align}
It is important to note that this is a pseudovector. 
...
\end{align}
Similarly, we can rule out a term of the form $p^\mu_{i}\,(p_{i}\cdot\epsilon)$. Note, Hemiticity also rules out terms proportional to $p^\mu_{i}\,(p_{f}\cdot\epsilon)$ or $p^\mu_{i}\,(p_{i}\cdot\epsilon)$.

\subsubsection{Lorentz decomposition of $\rho\gamma^\star\to\rho'$}

Now, we turn to the case where we have a stable spin-1 particle in the initial and final state. Once gain, the matrix element must be proportional to the polarization vector of the initial state, $\epsilon^{\mu}(p_i,\lambda_i)$, and complex conjugate of the final state, $\epsilon^{\mu}(\lambda_f;p_f)$. 


Let us list the possible vector structures allowed:
\begin{align}
\langle\rho'(\lambda_f,p_f)|{\mathcal{J}}^{\mu}|\rho(\lambda_i,p_i)\rangle
&=
(p_f+p)^\mu \,\epsilon_f^*\cdot \epsilon_i\, \widetilde{G}_1(Q^2,p_i^2,p_f^2)
\nn\\&
+(p_i-p_f)^\mu \,\epsilon_f^*\cdot \epsilon_i\,\widetilde{G}_2(Q^2,p_i^2,p_f^2)
\nn\\&
+\epsilon_i^\mu\, p_f\cdot \epsilon_f^*\,\widetilde{G}_3(Q^2,p_i^2,p_f^2)
\nn\\&
+\epsilon_f^{*\mu}\, p_f\cdot \epsilon_i\,\widetilde{G}_4(Q^2,p_i^2,p_f^2)
\nn\\&
+(p_f+p)^\mu \,(\epsilon_f^*\cdot p_i)\,(\epsilon_i\cdot p_f)\,\widetilde{G}_5(Q^2,p_i^2,p_f^2)
\nn\\&
+(p_i-p_f)^\mu \,(\epsilon_f^*\cdot p_i)\,(\epsilon_i\cdot p_f)\,\widetilde{G}_6(Q^2,p_i^2,p_f^2).
\end{align}
 In total we find six possible structures, where we excluded terms of the form $p\cdot \epsilon_i=p_f\cdot \epsilon_f^*=0$.

It is worth remarking about the presence of the terms of the form $\epsilon_f^{*\mu}\, p_f\cdot \epsilon_i$. Naively, one might guess that these transform as pseudovectors, but if we pick the rest frame of the initial state, we find that under parity this transforms as 
\begin{align}
...
\vec{p}_f \cdot \vec{\epsilon}_i.
\end{align}
The zero helicity component satisfies $\hat{e}_{-\lambda_f}=\hat{e}_{\lambda_f}$, while the other components satisfy $\hat{e}_{-\lambda_f}=-\hat{e}_{\lambda_f}$. So, we find that the $\lambda_f=\pm 1$ pieces transform like a vector under parity. 
...
\end{align}
Note, all kinematic terms in front $\widetilde{G}_1,\widetilde{G}_5$ and $\widetilde{G}_2,\widetilde{G}_6$ are respectively explicitly symmetric and antisymmetric in terms of the interchange of coordinate. This implies that $\widetilde{G}_1,\widetilde{G}_5$ and $\widetilde{G}_2,\widetilde{G}_6$ are respectively symmetric and antisymmetric. While the terms proportional to $\widetilde{G}_3$ and $\widetilde{G}_4$ are neither symmetric nor antisymmetric. Imposing the condition above, we find
\begin{align}
\epsilon_i^\mu\, p_f\cdot \epsilon_f^*\,\widetilde{G}_3(Q^2,p_i^2,p_f^2)
+\epsilon_f^{*\mu}\, p_f\cdot \epsilon_i\,\widetilde{G}_4(Q^2,p_i^2,p_f^2)
&=
\epsilon_f^{*\mu}\, p_f\cdot \epsilon_i\,\widetilde{G}_3(Q^2,p_f^2,p_i^2)
+\epsilon_i^{\mu}\, p\cdot \epsilon_f^*\,\widetilde{G}_4(Q^2,p_f^2,p_i^2)
\end{align}
Since $\epsilon_f^\mu$ and $\epsilon_i^\mu$ are independent of each other, we get
\begin{align}
\widetilde{G}_3(Q^2,p_i^2,p_f^2)=\widetilde{G}_4(Q^2,p_f^2,p_i^2).
\end{align}
...
\nn\\&
+\left[-(p_f\cdot\epsilon_i)\, p\cdot \epsilon_f^*\,
+(p_i\cdot\epsilon_f^{*})\, p_f\cdot \epsilon_i\,\right]\widetilde{G}_3(Q^2,p_i^2,p_f^2)
\nn\\&
...
\end{align}
This must vanish for all values of $p,p_f,\lambda,$ and $\lambda'$. If we pick the helicities, such that $\epsilon_f^*\cdot \epsilon_i=0$, we get 
\begin{align}
{(p_f^2-p_i^2)}\,\widetilde{G}_5(Q^2,p_i^2,p_f^2)
-Q^2 \,\widetilde{G}_6(Q^2,p_i^2,p_f^2)&=0\\
\Rightarrow 
\widetilde{G}_6(Q^2,p_i^2,p_f^2)&=\frac{(p_f^2-p_i^2)}{Q^2} \,\widetilde{G}_5(Q^2,p_i^2,p_f^2).
\end{align}
Using this constraint in Eq.~\ref{eq:charge_cons_rho}, we are then left with 
\begin{align}
...
\end{align}
Note, this is consistent with the observation made above that $\widetilde{G}_1,\widetilde{G}_5$ and $\widetilde{G}_2,\widetilde{G}_6$ are symmetric and antisymmetric.
...
\end{align}
{\raul For the case where $p_f^2=p_i^2$ this is consistent with the standard Lorentz decomposition of the matrix elements of a stable spin-1 particle, e.g., Eq.~(5) in Ref.~\cite{Shultz:2015pfa}. Also, Ref.~\cite{Jaus:2002sv} gives an expression, Eq. (2.1), which claims to be the most general form for a electroweak transition matrix element between two spin-1 particles with arbitrary masses. When the current is just the electromagnetic current, it states that only three form factors survive. This is reassuring but the relationship between their expression and this one is not immediately obvious. }
...
\end{align}
Under parity, this would transform to 
\begin{align}
...
+
P^{-\mu}\mathcal{F}_{2}(Q^2;E_i^*,\hat{k}_i;E_f^*,\hat{k}_f)
\nn\\
&+
...
\end{align}
Because $\mathcal{F}_{4}(Q^2;E_i^*,\hat{k}_i;E_f^*,\hat{k}_f)$ is antisymmetric with respect to the interchange of initial and final states, we might want to make this explicit by redefining it in terms of 
\begin{align}
\mathcal{F}_{4}(Q^2;E_i^*,\hat{k}_i;E_f^*,\hat{k}_f)
\equiv 
...
{\red Now I'm really concerned that this term should vanish if we set the initial and final state to be the same. This is very strange, cause otherwise it seems to contribute. Perhaps this will fine and what appears }

Again we find qualitative agreement between Eq.~\ref{eq:pipi_to_pipi} and that for the matrix elements of the $\rho\to\rho'$, Eq.~\ref{eq:rho_to_rho_FF}. Not only do we see that the two depend on just three Lorentz scalars, but also we find the first term to have the same exact kinematic dependence. This is quite nice and it partly explains why we had to consider two nondegenerate $\rho$ states. 

Once again, this amplitude is far more general than that of the $\rho\to\rho'$ process, since this describes any allowed process where the initial and final states are projected to any partial wave, not just $\ell=1$. In fact, one should be able to find that when the initial and final states are projected onto an S-wave, only one Lorentz scalar amplitude survives, leading to a similar expression as in Eq.~\ref{eq:pi_to_pi_FF}.  This already tells us that likely that in this limit, the terms proportional to $\mathcal{F}_{3}$ and $\mathcal{F}_{4}$ must vanish.
...
Here we verify the equivalence between Eq.~\ref{eq:pi_to_rho_FF} and Eq.~\ref{eq:pi_to_pipi} in the case where the final $\pi\pi$ is projected onto a $P$-wave. To do this, we follow steps presented in {\raul Ref..}

{\raul [this is a text dump from the $\pi\gamma\to\pi\pi$ paper appendix]
...
\includegraphics[scale=0.5]{not_in_iWdf}}
\hspace{.5cm}
\subfigure[]{
\label{fig:IWdf_res_pole}
\includegraphics[scale=0.5]{IWdf_res_pole}}
