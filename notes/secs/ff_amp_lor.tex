...
{\mh \begin{align}
\epsilon^{\mu}(p,\lambda)=
\left(\frac{\vec{p}\cdot\hat{e}_\lambda}{\vert {\vec p } \, \vert }, \ \hat{e}_\lambda + \frac{1}{\vec p^{\,2}} \left( \frac{p^0}{\vert \vec p \, \vert} - 1  \right) {\vec{p}\cdot\hat{e}_\lambda} \ \vec{p}\right) \,.
\end{align}
This satisfies the orthogonality via
 \begin{align}
\epsilon^{\mu}(p,\lambda) p_\mu = p^0 \frac{\vec{p}\cdot\hat{e}_\lambda}{\vert {\vec p } \, \vert }  -  \hat{e}_\lambda \cdot \vec p - \left( \frac{p^0}{\vert \vec p \, \vert} - 1  \right) \vec{p}\cdot\hat{e}_\lambda   = 0 \,,
\end{align}
and normalization via
\begin{align}
\epsilon^{\mu}(p, \lambda) \epsilon_{\mu}(p, \lambda) & =  ( \hat{p}\cdot\hat{e}_\lambda)^2 - \left [  \hat{e}_\lambda +   \left( \frac{p^0}{\vert \vec p \, \vert} - 1  \right) { {\hat p}\cdot\hat{e}_\lambda} \  {\hat p}\right] ^2  \\
& =  ( \hat{p}\cdot\hat{e}_\lambda)^2 - 1 -    \left( \frac{p^0}{\vert \vec p \, \vert} - 1  \right)^2 ( { {\hat p}\cdot\hat{e}_\lambda} )^2    - 2    \left( \frac{p^0}{\vert \vec p \, \vert} - 1  \right) ({ {\hat p}\cdot\hat{e}_\lambda}  )^2 \,, \\
& = -1 + ( \hat{p}\cdot\hat{e}_\lambda)^2 \left[ 1 - \frac{(p^0)^2}{\vec p\,^2} - 1 + 2 \frac{p^0}{\vert \vec p \, \vert}     - 2    \frac{p^0}{\vert \vec p \, \vert} + 2      \right ]
\end{align}
{\mh This still is not correct... needs to be resolved}
}
{\raul \begin{align}
\epsilon^{\mu}(p,\lambda)=
...
\end{multline}
where the sum in the first line runs over four distinct terms: $iii$, $ii \! f$, $i \! f \!  \! f$, $f \!  \! f \!  \! f$. Further decomposing the one-index $Z$ functions on the second line leads to a total of $10$ scalar form factors for the three-index quantity. However the second line leads only to terms redundant with those from $X$, $Y_i$ and $Y_f$ and dropping these leads to $4$ terms from $Z$. These are to be combined with $2$ each from $X$, $Y_i$ and $Y_f$ for a grand total of $10$. Of these $10$, $4$ vanish due to the identity $p_i\cdot \epsilon_i=p_f\cdot \epsilon_f^*=0$.}
...
{\mh Next we show that time-reversal invariance implies that all six scalar form factors are real numbers for all real values of $Q^2,p_i^2,p_f^2$. We begin by
considering a specific interpolator for the rho
\begin{equation}
\vert \rho^+(\lambda, p) \rangle = \epsilon_{\lambda,\mu}(p) \lim_{p^0 \to \omega_\textbf p} (1/i) [- (p^0)^2 + \textbf p^2 + m^2] \int d^4 x e^{- i p^0  x^0 + i \textbf p \cdot \textbf x} \ \overline d(x) \gamma^\mu u(x) \vert 0 \rangle \,.
\end{equation}
Acting with the time reversal operator on this state give
\begin{equation}
T \vert \rho^+(\lambda, p) \rangle = \epsilon^*_{\lambda,\mu}(p) \lim_{p^0 \to \omega_\textbf p} i [- (p^0)^2 + \textbf p^2 + m^2] \int d^4 x e^{ i p^0  x^0 - i \textbf p \cdot \textbf x} \ T \overline d(x) \gamma^\mu u(x)     T^{-1} \  \vert 0 \rangle
\end{equation}

\begin{align}
\langle\rho'(\lambda_f,p_f)|{\mathcal{J}}^{\mu}(0)|\rho(\lambda_i,p_i)\rangle & = \langle\rho'(\lambda_f,p_f)| T^{-1} T {\mathcal{J}}^{\mu}(0) T^{-1} T |\rho(\lambda_i,p_i)\rangle \,, \\
 & =  \langle\rho'(\lambda_f,p_f)| T^{-1} T {\mathcal{J}}^{\mu}(0) T^{-1} T |\rho(\lambda_i,p_i)\rangle
\end{align}}
