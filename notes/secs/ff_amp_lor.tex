...
\caption{Flowchart summarizing the process for extracting resonance form factors using the formalsim of Ref.~\cite{Briceno:2015tza}. {\mh [Check second update, now two analytic continuation arrows and colors]}}\label{fig:flowchart}
\end{center}
\end{figure*}
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\end{equation}}
 We begin with the elastic pion matrix elements, $\langle\pi(p_f)|{\mathcal{J}}^{\mu}|\pi(p_i)\rangle$, for the general case where pions are not on shell. This matrix element is a Lorentz vector, therefore it can only be proportional to linear combinations of the two vectors in the problem ($p_i^\mu, p_f^\mu$),
	...
	\end{align}
	{\ab where the functions $F_\pi$ and $F_{\pi,-}$ could {\em a priori} have non-vanishing imaginary parts.
	...
	{\mh [I think we can drop the following...] (Looking at $\mu =0$ in Eq.~(\ref{eq:t-rev-gpi}) we obtain the following relation
	\begin{eqnarray}
...
{\mh [I agree with Raul's suggestion here and have already implemented it.]}
...
Next implementing the identities $p_i\cdot \epsilon_i=p_f\cdot \epsilon_f^*=0$ we see that six terms survive, two from $X^\mu$ and $Z$ and one each from $Y_i^\mu$ and $Y_f^\mu$. 
%
...
%]}
%
% MH: I agree! and made the changes above
%
%
%We deduce that Lorentz invariance alone restricts the decomposition to $6$ distinct, nonzero scalar form factors. 
...
{\mh [It is important to note that our goal here is to find a relation between $\langle \alpha \vert \mathcal O \vert \beta \rangle$ and $\langle \alpha \vert \mathcal O \vert \beta \rangle^*$. If we find a relation between $\langle \alpha \vert \mathcal O \vert \beta \rangle$ and $\langle \beta \vert \mathcal O \vert \alpha \rangle^*$ then we just get a statement about operator hermiticity but not about the reality of the matrix element.]}
]
}
...
\end{equation}
A similar argument can be used to show that all six scalar form factors satisfy this condition. {\mh [Check this!]}
...
We point the reader to Ref.~\cite{Briceno:2016kkp} for details for how this can then be used to find the equivalence between Eq.~\ref{eq:pi_to_rho_FF} and Eq.~\ref{eq:pi_to_pipi}.
...
%
%{\raul [not sure what's the point of this]
%\begin{align}
%\langle \pi(p_f)|\mathcal{J}^\mu|\rho(\lambda; p_i)\rangle
%=
%\epsilon^{\mu\alpha\beta\gamma}p_{f,\alpha}p_{i,\beta}\epsilon_\gamma F_1(Q^2,p_i^2,p_f^2)
...
\subsubsection{Equivalence between $\gamma^\star\sigma\to\sigma$ and $\gamma^\star(\pi\pi)_{\ell=0}\to(\pi\pi)_{\ell=0}$}
...
We can simplify this by first writing the angular dependence of the amplitudes in terms of spherical harmonics, 
\begin{align}
...
%\begin{eqnarray}
%\int d\hat{{\bf k}}^\star_id\hat{{\bf k}}^\star_f{\cal F}_1(Q^2;E_i^\star,{\hat k}^\star_i;E_f^\star,\hat{k}^\star_f)&=&4\pi\int d\hat{{\bf k}}^\star_id\hat{{\bf k}}^\star_f Y^*_{l^\prime m^\prime}(\hat{k}^\star_f){\cal F}_{1;l^\prime m^\prime;lm}(Q^2;E_i^\star;E_f^\star)Y_{lm}(\hat{k}^\star_i) \\
...
{\raul[I would like to first write down the final result and then derive it...but I don't know it yet]}
...
Note, this has the same Lorentz structure as the first and last terms in Eq.~\ref{eq:rho_to_rho_FF},
\begin{align}
\langle\rho'(\lambda_f,P_f)|{\mathcal{J}}^{\mu}|\rho(\lambda_i,P_i)\rangle
&\supset
...
Next, let us consider part of the second term
\begin{align}
&\int d\hat{\bf k}^\star_i\int d\hat{\bf k}^\star_f
\frac{Y_{1\lambda_i}(\hat{k}_i^\star)}{\sqrt{4\pi}}
...
{\raul  First we establish that $P_\mu X^\mu = 0$,
\begin{align}
	P_\mu X^\mu 
 	&= \int \! d\hat{\mathbf{k}} \, Y_{1\lambda}(\hat{\mathbf{k}})\, \left[ \frac{\Epipi^2}{2}  - \frac{P^2}{2} \right] = 0,
\end{align}
...
%&\int d\hat{\bf k}_i^\star\int d\hat{\bf k}_f^\star Y^*_{1\lambda_f}(\hat{k}_f^\star) Y_{1\lambda_i}(\hat{k}_i^\star)\langle\pi(R_f{\bf k}_f^\prime)\pi({\bf P}_f-R_f{\bf k}_f^\prime)|{\cal J}^\mu(0)|\pi(R_i{\bf k}_i^\prime)\pi({\bf P}_i-R_i{\bf k}_i^\prime)\rangle\, =\\
%&\frac{1}{4\pi}\int d\hat{\bf k}_i^\star\int d\hat{\bf k}_f^\star Y^*_{1\lambda_f}(\hat{k}_f^\star) Y_{1\lambda_i}(\hat{k}_i^\star)\bigg[ \left(P^{+\mu}+\frac{(E_i^{\star 2}-E_f^{\star 2})}{Q^2} P^{-\mu}\right)\mathcal{F}_{1}(Q^2;E_i^\star,\hat{k}^\star_i;E_f^\star,\hat{k}^\star_f)
%\nn\\
%&+
%\left(k^{+\mu}+\frac{(P^-\cdot k^{+})}{Q^2} P^{-\mu}\right)
%\mathcal{F}_{3}(Q^2;E_i^\star,\hat{k}^\star_i;E_f^\star,\hat{k}^\star_f)
...
%\begin{eqnarray}
%&\sum_{lm,l^\prime m^\prime}\int d\hat{\bf k}_i^\star\int d\hat{\bf k}_f^\star Y^*_{1\lambda_f}(\hat{k}_f^\star)Y_{l^\prime m^\prime}(\hat{k}_f^\star)  Y_{1\lambda_i}(\hat{k}_i^\star)Y^*_{lm}(\hat{k}_i^\star) \mathcal{F}_{1;l m;l^\prime m^\prime}(Q^2;E_i^\star;E_f^\star)\nonumber\\
...
%& Y^*_{1\lambda_f}(\hat{k}_f^\star)Y_{l^\prime m^\prime}(\hat{k}_f^\star)  Y_{1\lambda_i}(\hat{k}_i^\star)Y^*_{lm}(\hat{k}_i^\star) \mathcal{F}_{3;l^\prime m^\prime;l m}(Q^2;E_i^\star;E_f^\star)\label{eq:full_dec}
%\end{eqnarray}
...
%\begin{eqnarray}
%\int d\hat{{\bf k}}^\star_i Y^*_{l m}(\hat{k}^\star_i)Y_{1\lambda_i}(\hat{k}_i^\star)k_i^\prime&=&\int d\hat{{\bf k}}^\star_i Y^*_{l m}(\hat{k}^\star_i)Y_{1\lambda_i}(\hat{k}_i^\star)\begin{bmatrix} \sqrt{\pi}Y_{00}E_i + \tfrac{|\textbf{P}_i|}{\omega_\pi}\sqrt{\frac{\pi}{3}}|{\bf k}_i^\star|Y_{10}(\hat{k}^\star_i) \\ \sqrt{\frac{2\pi}{3}}|{\bf k}_i^\star| \left[Y_{1,-1}(\hat{k}_i^\star)-Y_{11}(\hat{k}_i^\star)\right]\\ i\sqrt{\frac{2\pi}{3}}|{\bf k}_i^\star|\left[Y_{1-1}(\hat{k}_i^\star)+Y_{11}(\hat{k}_i^\star)\right] \\ \sqrt{\pi}Y_{0,0}|{\bf P}_i| + \tfrac{E_i}{\omega_\pi} \sqrt{\frac{\pi}{3}}|{\bf k}^\star_i|Y_{10}(\hat{k}_i^\star) \end{bmatrix}
%\end{eqnarray}
%
